...
The final limitation is that maistream OOP languages use a class-based static
...
Exactly the same situation arises when using inheritance. In Scala, if we try
to create a trait parametrized by an unknown supertype, for example
\lstinline[language=scala]{trait T[A] extends A}, we face problems. Here the
intention would be to capture, once and for all, a family of traits that extend
