...
\bruno{Should we say something about structural typing, purely
  functional? if so, where? here? the introduction?} \jeremy{moved to
  introduction, page 2, line 25}
...
and \lstinline{y}.\jeremy{say \name only supports Double}
...
own field \lstinline{radius}.\bruno{Should we talk about the second option
  here?} \jeremy{it's mentioned later on.}
...
\lstinputlisting[linerange=5-16]{../examples/evenOdd.txt}% APPLY:linerange=EVEN_ODD
...
\lstinputlisting[linerange=47-48]{../examples/box.txt}% APPLY:linerange=BUTTON_DEF

Similarly we define functions for buttons as well.
\lstinputlisting[linerange=52-59]{../examples/box.txt}% APPLY:linerange=BUTTON_FNS
...
\lstinputlisting[linerange=63-65]{../examples/box.txt}% APPLY:linerange=ROUNDBUTTON_DEF

\noindent Finally we create a round button on the fly and test its functionality
as follows:
\lstinputlisting[linerange=121-124]{../examples/box.txt}% APPLY:linerange=ROUNDBUTTON_TEST
...
\lstinputlisting[linerange=70-73]{../examples/box.txt}% APPLY:linerange=ASOVAL_DEF
...
\lstinputlisting[linerange=78-79]{../examples/box.txt}% APPLY:linerange=CONFLICT_DEF
...
by \name again.
\lstinputlisting[linerange=85-86]{../examples/box.txt}% APPLY:linerange=CONFLICT_RESOLVE
...
\lstinputlisting[linerange=101-104]{../examples/box.txt}% APPLY:linerange=CIRCLE_FNS2
...
\lstinputlisting[linerange=90-97]{../examples/box.txt}% APPLY:linerange=NORM_DEF
...
\lstinputlisting[linerange=108-110]{../examples/box.txt}% APPLY:linerange=POINT_FUNC
...
in.
\lstinputlisting[linerange=114-114]{../examples/box.txt}% APPLY:linerange=ROUNDBUTTON_TEST2
...
disjoint to the type \lstinline{A}. Thirdly, the value of the intersection type
...
two arbitrary values to be merged, with the resulting type being an intersection
type.
