...
\begin{tabular}{lrclr}
  ...
  Program & $[[jpgm]]$ & ::= & $[[jdecl1 .. jdecln e]]$ & $\mathit{Function}$ \\
...
\begin{figure}[t]
  ...
  \begin{tabular}{cc}
    \begin{subfigure}[t]{0.45\textwidth}
      \centering
      \lstinputlisting[linerange=141-150]{../examples/visitor2.txt}% APPLY:linerange=BASE_DEF
    \end{subfigure}
    &
    \begin{subfigure}[t]{0.45\textwidth}
      \centering
      \lstinputlisting[linerange=122-131]{../examples/visitor2.txt}% APPLY:linerange=INTERFACE
    \end{subfigure}
  \end{tabular}
...
\lstinputlisting[linerange=156-162]{../examples/visitor2.txt}% APPLY:linerange=BASE_EVALUATOR
...
\begin{figure}[t]
  \centering
  \lstinputlisting[linerange=183-189]{../examples/visitor2.txt}% APPLY:linerange=PRETTY_BASE
...
\begin{figure}[t]
  \centering
  \lstinputlisting[linerange=194-204]{../examples/visitor2.txt}% APPLY:linerange=TYPECHECKER_BASE
...
\lstinputlisting[linerange=246-251]{../examples/visitor2.txt}% APPLY:linerange=CONDITIONAL
...
% \lstinputlisting[linerange=261-279]{../examples/visitor2.txt}% APPLY:linerange=EVALUATOR_CONDITIONAL
...
% \lstinputlisting[linerange=284-290]{../examples/visitor2.txt}% APPLY:linerange=PRETTY_CONDITIONAL
% \lstinputlisting[linerange=296-330]{../examples/visitor2.txt}% APPLY:linerange=TYPECHECKER_CONDITIONAL
...
\lstinputlisting[linerange=344-347]{../examples/visitor2.txt}% APPLY:linerange=VARIABLE
...
\lstinputlisting[linerange=357-363]{../examples/visitor2.txt}% APPLY:linerange=EVALUATOR_VARIABLE
...
argument expression\footnote{For simplicity, we only allow functions with one
  argument.}.
\lstinputlisting[linerange=391-396]{../examples/visitor2.txt}% APPLY:linerange=TOP_LEVEL
...
now.
\lstinputlisting[linerange=474-484]{../examples/visitor2.txt}% APPLY:linerange=FINAL_TEST
...
\lstinputlisting[linerange=488-499]{../examples/visitor2.txt}% APPLY:linerange=SUPER_DEF
...
\lstinputlisting[linerange=503-504]{../examples/visitor2.txt}% APPLY:linerange=TEST_TEST
