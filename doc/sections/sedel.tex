...
\lstinputlisting[linerange=4-6]{../examples/visitor.txt}% APPLY:linerange=ALGEBRA_DEF
...
\lstinputlisting[linerange=28-28]{../examples/visitor.txt}% APPLY:linerange=EVAL_DEF
Using \lstinline{IEval}, we can define a trait that implements the evaluation
rule for each variant:
\lstinputlisting[linerange=32-34]{../examples/visitor.txt}% APPLY:linerange=EVAL_IMPL
...
\lstinputlisting[linerange=10-10]{../examples/visitor.txt}% APPLY:linerange=MKVALUE_DEF
...
\lstinputlisting[linerange=14-14]{../examples/visitor.txt}% APPLY:linerange=CHURCH_DEF
...
Algebras are closely related to internal
visitors~\cite{}\bruno{missing reference: my 2008 OOPSLA paper}. However, \emph{Object
...
\lstinputlisting[linerange=68-68]{../examples/visitor.txt}% APPLY:linerange=EXPRESSION1_EG
...
\bruno{explanation for big lambda/lambda required.}
...
\lstinputlisting[linerange=39-42]{../examples/visitor.txt}% APPLY:linerange=SUB_DEF
...
\lstinputlisting[linerange=48-48]{../examples/visitor.txt}% APPLY:linerange=PRINT_DEF
The implementation is straightforward:
\lstinputlisting[linerange=53-56]{../examples/visitor.txt}% APPLY:linerange=PRINT_IMPL
...
\lstinputlisting[linerange=63-64]{../examples/visitor.txt}% APPLY:linerange=EVAL_PRINT
...
%\lstinline{ExtExp}:
\lstinputlisting[linerange=71-72]{../examples/visitor.txt}% APPLY:linerange=EXPRESSION2_EG
...
\lstinputlisting[linerange=82-86]{../examples/visitor.txt}% APPLY:linerange=VISITOR_EG
...
That is it. None of the boilerplate in other approaches~\cite{}~\bruno{reference}, or type-unsafe
meta-programming techniques~\cite{}~\bruno{reference} of other approaches are needed! Three points
...
\lstinputlisting[linerange=68-71]{../examples/visitor3.txt}% APPLY:linerange=COMBINE1_TEST
...
is rejected:
\lstinputlisting[linerange=77-77]{../examples/visitor.txt}% APPLY:linerange=EXPRESSION_WRONG
...
\lstinputlisting[linerange=18-23]{../examples/visitor.txt}% APPLY:linerange=DC_DEF
